...
void main()
{
	...
	if (HasDiffuseTexture)
		gl_FragData[0]= vec4(DiffuseColor.xyz*texture2D(DiffuseTexture, TexCoord).xyz, enc.x);
	else gl_FragData[0]= vec4(DiffuseColor.xyz, enc.x);
	gl_FragData[1] = vec4(EyespacePosition.xyz, enc.y);
