...
void main()
{
	...
	else gl_FragData[0]= vec4(DiffuseColor, enc.x);
	gl_FragData[1] = vec4(EyespacePosition, enc.y);
	if (HasSpecularTexture)
    	gl_FragData[2] = vec4(float(BLINNPHONG_MATERIAL_ID), SpecularColor*texture2D(SpecularTexture, TexCoord).xyz);
	else gl_FragData[2] = vec4(float(BLINNPHONG_MATERIAL_ID), SpecularColor);
