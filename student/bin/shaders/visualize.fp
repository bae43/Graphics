...
void main()
{
	...
	gl_FragColor = vec4(0.0);
	 int material =  int((materialParams1.x < 0.0 ? materialParams1.x : materialParams1.x));
	 
	 if (VisMode == NORMALS_MODE){
	 	gl_FragColor = vec4(normal.xyz*0.5 + 0.5, 0.0);
	 }
	 else if (VisMode == TANGENTS_MODE && material==ANISOTROPIC_WARD_MATERIAL_ID){
	 	vec3 tangent = decode(vec2(materialParams2.xy));
	 	gl_FragColor = vec4(tangent.xyz * 0.5 + 0.5, 0.0);
	 }
	 else if (VisMode == BITANGENTS_MODE && material==ANISOTROPIC_WARD_MATERIAL_ID){
	 	float material = materialParams1.x;
	 	vec3 tangent = decode(vec2(materialParams2.xy));
	 	float bitangent_sign = (material < 0.0 ? -1.0 : 1.0);
	 	vec3 bitangent = bitangent_sign * cross(normalize(normal), normalize(tangent));  
	 	gl_FragColor = vec4(bitangent.xyz * 0.5 + 0.5, 0.0);
	 }
}
