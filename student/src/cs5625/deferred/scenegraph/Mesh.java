...
public abstract class Mesh implements OpenGLResourceObject
{
	...
	protected void computeAndAccumulateTangentVectors(int i1, int i2, int i3, float tan1[], float[] tan2) {
		...
		matQ.setRow(0, new GVector(q1));
		matQ.setRow(1, new GVector(q2));
		...
		// Inverse Matrix		
				GMatrix inv_matrix = new GMatrix(2, 2);
				inv_matrix.setElement(0, 0, t2);
				inv_matrix.setElement(0, 1, -t1);
				inv_matrix.setElement(1, 0, -s2);
				inv_matrix.setElement(1, 1, s1);
				// Scalar
				double constant = 1/(s1*t2 - s2*t1);
				// Tangent and Bitangent calculations
				GMatrix bi_tan_matrix = new GMatrix(2, 3);
				bi_tan_matrix.mul(inv_matrix, matQ);
				bi_tan_matrix.getRow(0, tangent);
				bi_tan_matrix.getRow(1, bi_tangent);
				tangent.scale(constant);
				bi_tangent.scale(constant);
	...
	protected FloatBuffer averageAndNormalizeAllTangentVectors(float tan1[], float[] tan2) {
		...
		for (int vIndex = 0; vIndex < getVertexCount(); ++vIndex)
		{
			...
			tangent_orth.x = (tangent.x - normal.x*dot);
			tangent_orth.y = (tangent.y - normal.y*dot);
			tangent_orth.z = (tangent.z - normal.z*dot);
			...
			cross.cross(normal, tangent);
			cross.normalize();
			float dot_product = cross.dot(bitangent); 
			handedness = (dot_product < 0.0F ? -1.0F : 1.0F);
